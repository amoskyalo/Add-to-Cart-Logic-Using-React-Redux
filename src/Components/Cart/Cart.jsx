...
import { cartData, removeCart } from './CartSlice';
import { GrFormAdd } from 'react-icons/gr';
import { AiOutlineMinus, AiFillDelete } from 'react-icons/ai';
import EmptyCart from './EmptyCart';
import { commonData } from './CartSlice';
...
const Cart = () => {
    ...
    const thisData = useSelector(commonData);
        ...
        <div className="flex flex-row space-x-1">
            <div className="flex flex-col space-y-4 w-3/4 bg-white p-4 rounded">
                <div className="flex flex-row justify-between border-b">
                    ...
                    <h2 className='text-xl font-bold'>{data.length} {data.length <= 1 ? "Item" : "Itmes"}</h2>
                </div>
                {!data.length ? <EmptyCart /> : 
                data.map((cartItem, i) =>(
                    <div className="flex flex-row justify-between items-center w-full" key={i}>
                       <div className="flex bg-greyColor rounded-full p-1">
                       <img src={cartItem.img} alt="" className='h-24 w-24 object-contain rounded-full'/>
                       </div>
                        ...
                        <div className="flex flex-row space-x-2 items-center">
                            <AiOutlineMinus className='border text-2xl cursor-pointer'/>
                            <p className='text-base'>0</p>
                            <GrFormAdd className='border text-2xl cursor-pointer'/>
                        </div>
                        <div className="flex" onClick={() => removeCartFunc(cartItem.id)}>
